...
from bgi.bert.layers import *
from bert.snippets import delete_arguments
from bert.snippets import is_string
...
class BERT(Transformer):
    ...
    def __init__(
        ...
        multi_feature_inputs: list = [],
        multi_embedding_inputs: list = [],
        multi_embedding_dims: list = [],
        **kwargs  # 其余参数
    ):
        ...
        self.multi_feature_inputs = multi_feature_inputs
        self.multi_embedding_inputs = multi_embedding_inputs
        self.multi_embedding_dims = multi_embedding_dims
    ...
    def get_inputs(self):
        ...
        s_in = Input(shape=(self.sequence_length, 1), name='Input-Segment')

        if self.custom_position_ids:
            p_in = Input(shape=(self.sequence_length,), name='Input-Position')
            return [x_in, s_in, p_in]
        elif len(self.multi_feature_inputs) > 0 or len(self.multi_embedding_inputs):
            inputs = [x_in, s_in]
            index = 0
            for ii in range(len(self.multi_embedding_inputs)):
                t_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Custom-Type-{}'.format(index))
                inputs.append(t_in)
                index += 1
            for ii in range(len(self.multi_feature_inputs)):
                t_in = keras.layers.Input(shape=(self.multi_feature_inputs[ii],), name='Input-Custom-Type-{}'.format(index))
                inputs.append(t_in)
                index += 1
            return inputs
        else:
    ...
    def apply_embeddings(self, inputs):
        ...
        # )
        embedding_inputs = [x]
        if len(self.multi_embedding_inputs) > 0:
            raw_embedding_input_len = len(embedding_inputs)
            for ii in range(raw_embedding_input_len, len(self.multi_embedding_inputs) + raw_embedding_input_len):
                if ii < len(inputs):
                    input = inputs[ii]
                    type = self.apply(
                        inputs=input,
                        layer=Embedding,
                        input_dim=self.multi_embedding_dims[ii - raw_embedding_input_len],
                        output_dim=self.embedding_size,
                        embeddings_initializer=self.initializer,
                        name='Embedding-Custom-{}'.format(ii - raw_embedding_input_len)
                    )
                    embedding_inputs.append(type)
        if len(embedding_inputs) > 1:
            x = self.apply(inputs=embedding_inputs, layer=Add, name='Embedding-Token-Segment')
        else:
            x = embedding_inputs[0]

        # x = self.apply(
        #     inputs=[x, s],
        #     layer=keras.layers.Multiply,
        #     name='Multiply'
        # )
